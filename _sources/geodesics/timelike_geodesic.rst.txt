...
As for the case of the null geodesics, the :math:`u^\phi` component of the 4-velocity has, in principle, no direct physical meaning. For this reason, if one wants to assign an initial spatial velocity on a physical direction in space and with a given initial spatial velocity relative to a specific space-time observer, the :py:class:`.Observer` class is the most appropriate, for which we refer to the :doc:`../define_observer` tutorial.